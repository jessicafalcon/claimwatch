...
| **2 — One scraper end to end** | App Store review feed (public JSON, ToS-friendly — recommended over Opinion Assurances) → raw → staging → `reviews_per_month` mart. Strict parse of the feed shape. Politeness settings in one place. (Built: the host's `robots.txt` disallows the feed path for every crawler, so the source is declared and not fetched — DECISIONS → Phase 2 and Gotchas; the metric is a pinned query, not a mart — in `pipeline/build.py` since Phase 9e folded `pipeline/metrics.py` away; real rows move to 3a.) | `make rebuild FIXTURE=app-store && make idempotency-check FIXTURE=app-store` (as run then; `ROWS=samples` since Phase 3a; was `make rebuild` on real rows — moved to 3a by fix amendment A1) | + security-reviewer (network) |
