...
| 3 | `tests/test_cli.py::test_cli_refuses_a_corpus_input_on_the_cloud_target_on_every_path` — parametrised over `rebuild` × four stages and `idempotency-check`: `TARGET=snowflake ROWS=captured\|samples` is one line, exit 2, nothing connected (the fake's `connect` never called); `publish`'s corpus refusal is `tests/test_metabase_apply.py::test_export_command_refuses_a_target_outside_wired_by_name` (round 2 code-reviewer #3: publish is pinned by the metabase test, not this parametrisation) |
