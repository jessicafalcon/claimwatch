...
| `idempotency-check TARGET=` | → `duckdb` | refused by name | refused by name | the same | n/a | `tests/test_cli.py::test_cli_idempotency_check_refuses_non_duckdb_target` (renamed: refuses a target outside `WIRED`); the corpus-input test above |
