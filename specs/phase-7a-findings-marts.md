...
allowlist is in CLAUDE.md → Conventions; this phase is DuckDB + stdlib SQL).
...
4. **One segment per source.** Every classified review resolves to exactly one
   segment via `select distinct source, segment from raw_source_pages`; a source
   declared with two segments is caught, never silently double-counting a
   review across segments. *Evidence: row 4.*
...
| 4 | `tests/test_theme_share.py::test_two_segments_one_source_is_refused` |
...
| For all reviews, the review's `source` maps to exactly one segment, so no review is counted under two segments. | `tests/test_theme_share.py::test_two_segments_one_source_is_refused` — a source declared with two segments. |
...
- **Segment attaches by the `source` slug, not `source_url`.** Segment/channel
  are attributes of a source declaration, so `select distinct source, segment
  from raw_source_pages` is the correct grain; it matches on synthetic and real
  alike. Rejected: the `source_url` join documented in `raw_source_pages`
  (0/39 on synthetic; "fixing" the fixtures would inject real-brand URLs into
  the brand-free corpus). Correct the `raw_source_pages` header to match.
...
- `sql/raw/raw_source_pages.sql` — header correction: the review→segment join is
  by `source`, not `source_url` (data unchanged).
- `sql/staging/stg_reviews.sql` — header: it feeds the marts via
  `stg_classified_reviews` by `(source, external_id)` (join key corrected).
- `pipeline/build.py` — `write_classified_reviews`; exclude the two theme-share
  marts from the generic marts loop.
