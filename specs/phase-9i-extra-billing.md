...
```
make review-gate SPEC=specs/phase-9i-extra-billing.md && make share-ameli && make rebuild ROWS=synthetic && make study && git diff --exit-code -- data/ameli/fee_split.csv study/friction_ledger.html
```
...
- `make share-ameli` — recompute the fee split from `fixtures/ameli/` and
...
1. **The second source is declared and read to a declared shape.** One
   ...
   byte-order mark on the header), keeps the year's four national family rows,
   and refuses by name a missing column, a family missing or repeated, or a
   family whose total is a suppressed token (`NS`, `NC`) or not an ASCII whole
   number — a dropped family would silently move the national total; every
   other row is dropped and counted. *Evidence: row 1.*
2. **The fixture is the four rows of one year, frozen; the artifact is plain
   ...
   writes `fixtures/ameli/honoraires-national.csv` (four columns:
   `annee;profession_sante;hono_sans_depassement_totaux;depassements_totaux`,
   four rows) and its `MANIFEST.sha256`; `make share-ameli` reads the fixture,
...
3. **The artifact's name set is one public constant and its reader is
   ...
   requires and the tracked-files test reads; `read_fee_split` parses values in
   the repo's shared shapes (`DECIMAL_SHAPE`, the ASCII count shape), refuses a
   missing, extra or duplicate name, a non-numeric or non-finite value, a share
   outside `[0, 1]`, a negative total and an oversized file, each by name, and
   returns the same split whatever the row order. *Evidence: row 3.*
4. **The model carries the row and nothing already shown moves.**
   `parameters(fit, fees)` gains `extra_billing_share` after `emp_mean` —
   sourced, cited to the artifact and the dataset, default the all-families
   share, `low`/`high` the lowest and highest family share (a real range, not a
   fixed mark, so the read-figure BACKLOG row's trigger does not fire); no
   formula reads it; every pinned parameter cell, output, crossover and Beat 4
...
5. **The page shows the row and says only what its cells show.** B3.3 renders
   ...
   sourced row; the panel's blurb names it; its note (data in `study/text.py`)
   says the default is the four families together and the range the lowest and
   highest family, and that the Assurance Maladie reimburses none of the part
   above the tariff, so that part is absent from the reimbursement cells the
   fit is built on — no figure the marts do not hold; B3.3's sources gain the
...
6. **The records say so.** SPEC.md B3.3 names the row; BACKING.md's B3.3
   source cell names the dataset and a paragraph says what data.ameli holds,
   what it cannot supply and what the row shows; README Beat 3 names it in one
   sentence and the glossary gains *extra billing*; the BACKLOG row is struck
   with its trigger reinterpreted and one row opened on the complementary-side
   claim size; DECISIONS records the dataset facts, the robots decision and
   the brief §7 clause as read. *Evidence: row 6.*
...
| 2 | `make slice-ameli YEAR=2024` (developer-run, once); `make share-ameli` prints the split; `tests/test_fee_split.py::test_shares_recomputed_by_hand`, `tests/test_fee_split.py::test_write_fee_split_is_byte_identical_on_rerun`, `tests/test_fee_split.py::test_artifact_equals_recompute`, `tests/test_fee_split.py::test_freeze_manifest_matches_sha256`, `tests/test_fee_split.py::test_share_ameli_writes_and_prints`, `tests/test_fee_split.py::test_share_ameli_missing_fixture_is_a_message` |
...
| 4 | `tests/test_cost_model.py::test_extra_billing_share_row_spans_the_family_extremes_and_no_formula_reads_it`, `tests/test_cost_model.py::test_formulas_evaluate_to_the_pins` (unchanged pins), `tests/test_cost_model.py::test_each_scenario_changes_only_its_toggled_parameters`, `tests/test_model_marts.py::test_params_mart_has_one_row_per_parameter`, `tests/test_model_marts.py::test_three_marts_filled_on_every_input`, `tests/test_model_marts.py::test_two_rebuilds_identical_model_mart_rows`, `tests/test_model_marts.py::test_read_model_fit_refuses_a_malformed_artifact` (extended to the fee split), `tests/test_guardrail_sim.py` and `tests/test_sim_marts.py` (unchanged pins) |
...
|---|---|
| 1. For all exports the slice reads, a kept row is one of the year's four national family rows, each family exactly once with two ASCII whole-euro totals; a family missing, repeated or suppressed refuses by name, never a partial total. | `tests/test_fee_split.py::test_slice_refuses_a_missing_repeated_or_suppressed_family_by_name` — an export lacking `Sages-femmes`, one with it twice, one with `NC` in a total; `::test_slice_keeps_the_four_national_family_rows_of_the_year` — a sub-group row, another year, another region are not kept |
...
| 3. For all runs of `share-ameli` over the same fixture, the artifact's bytes are identical, and the committed artifact equals the recompute (no clock, no RNG, closed-form). | `tests/test_fee_split.py::test_write_fee_split_is_byte_identical_on_rerun`, `::test_artifact_equals_recompute` |
| 4. For all artifacts, the reader accepts exactly the closed name set with values in the shared shapes and the declared domains, and returns the same split in any row order; anything else refuses by name — never a silent default, never a traceback downstream. | `tests/test_fee_split.py::test_read_fee_split_refuses_unknown_missing_or_non_numeric_names`, `::test_read_fee_split_refuses_a_share_outside_zero_one_a_negative_total_and_an_oversized_file`, `::test_read_fee_split_is_order_independent`; `tests/test_model_marts.py::test_read_model_fit_refuses_a_malformed_artifact` |
...
| 8. For all values of `YEAR`, the value is validated to a closed four-digit shape before anything is read and never becomes a path; an empty, path-escaping, metacharacter or environment-set value refuses in one line. | `tests/test_fee_split.py::test_valid_year_refuses_bad_shapes`, `::test_slice_ameli_year_validation_refuses_before_any_read`; `tests/test_makefile.py::test_pipeline_variables_reach_python_as_one_literal` (gains `slice-ameli`/`YEAR`) |
| 9. For all of this phase, no command touches the network: the export is a file a person placed; the tracked artifact and the page are reproduced offline. | `tests/test_fee_split.py::test_opendata_fees_imports_no_network_module` (no `urllib`/`httpx` import in the new module); `make review-gate` offline |
...
- **The quantity is the extra-billing share over the four top-level families
  ...
  and `high` the lowest and highest family share — a range read from the data,
  like the ±2-SE fit rows are, and stated as such in the note. The four
  families are the whole (the 38 national labels nest under them); the closed
  label set lives in the source declaration. *Rejected: a lognormal over
  per-practitioner mean fees (a practitioner-income curve, not a claim cost —
  the BACKLOG row's premise, which the dataset does not support); a fixed mark
  with the spread in prose (a figure in a note must be a mart cell; the
  read-figure BACKLOG row would fire for no gain); five rows, one per family
  (clutter in B3.3 for one fact).* Satisfies invariants 2 and 5.
...
- **One new module, one extended declaration.** `opendata/sources.py` becomes
  ...
  family labels with their artifact slugs, `valid_year`); `opendata/fees.py`
  ...
  in one file because the arithmetic is two sums and a division. `ingest/`
  and `models/` import nothing new. *Rejected: a second package; a
  `fetch.py`-style module with nothing to fetch.* Satisfies invariants 1, 3.
- **The artifact is `name,value`, numbers only, one closed name set.**
  ...
  copy). Euros whole (the source publishes whole euros), shares at six places
  ...
  invariants 3, 4.
- **The model takes the split as a second small input beside the fit; no
  formula reads it.** `cost_model.FeeSplit(year, share, low, high)` is what the
  model needs; `parameters(fit, fees)` and the callers that build the value
  set take it; `pipeline/build.py` gains the one reader that lifts the
  artifact into it (the `read_model_fit` sibling), so `models/` still reads no
  ...
  a data.ameli figure — name drift); a formula that multiplies the insurer's
...
- `opendata/fees.py` — new: `read_national_families`, `write_fixture`,
...
- `fixtures/ameli/honoraires-national.csv`, `fixtures/ameli/MANIFEST.sha256`
  — the frozen four-row slice (first freeze).
- `data/ameli/fee_split.csv` — the tracked artifact; `.gitignore` gains
  `!data/ameli/`.
- `pipeline/cli.py` — `slice-ameli` (`YEAR`), `share-ameli`; the `model` and
  `simulate` paths read the split; `pipeline/build.py` — the split reader,
  `write_model_marts` / `write_sim_marts` take it.
- `Makefile` — `slice-ameli`, `share-ameli`; `.PHONY`; help lines.
- `models/cost_model.py` — `FeeSplit`, `parameters(fit, fees)` and the
  callers, the citation; `models/guardrail_sim.py` — its callers' signature
...
- [ ] `BACKLOG.md` — *data.ameli practitioner-fee distributions are not
  ...
  *The claim-cost anchor is the public insurer's reimbursement; a complementary
  ...
  reviewer asks for the complementary-side claim size — DAMIR's fee and base
  columns can supply the split per cell); the DAMIR-refresh row's trigger
  extended to name the fee split's vintage; the count updated
- [ ] LESSONS.md — none until a review round reports a correctness finding; then backtick it and the fix commit writes the row
- [ ] `CLAUDE.md` — Current status; Commands (`slice-ameli`, `share-ameli`;
  the `confirm` set unchanged); Repo map (`opendata/fees.py`,
  `fixtures/ameli/`, `data/ameli/`); BACKLOG count
- [ ] `BACKING.md` — B3.3's source cell gains `data-ameli-honoraires`; a
  paragraph "On the `data-ameli-honoraires` upstream (B3.3)" beside the DAMIR
  one; no row's tag changes
- [ ] `SPEC.md` — Beat 3: B3.3's sentence names the share and its family
  range (no chart or beat changes; the glossary is at its cap of ten, so the
  term is defined in the panel's note)
- [ ] `README.md` — Beat 3: one sentence naming the share; the glossary's
  tenth term, *extra billing*
...
(Phase 3a): one Python process validates `YEAR` against a closed four-digit
shape, then reads the cache file at a constant path and filters rows on the
...
rewrites the same fixture and manifest; `share-ameli` rewrites the same bytes.
...
- **coherence-auditor** at exit: "the four fit rows … 16 rows" moved to 17
  ...
  Repo map names `opendata/fees.py`; the BACKLOG row struck and the new row
...
- Stack risk (verify in the first hour, with the developer's downloaded file
  in hand): the export's delimiter and header as the browser saves them (the
  ...
  only). STOP and report before any workaround; findings → DECISIONS.md →
  Gotchas.
...
- **A brief §7 edit** — the clause "data.ameli for practitioner-level fees" as
  a simulator draw source does not hold for this dataset; DECISIONS records
  the reading, and whether the brief's sentence changes is the developer's
  call, not this phase's diff.
