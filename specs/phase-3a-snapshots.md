...
the one parser here is stdlib `json` over a page's machine-readable block;
`pyyaml` stays pre-approved and unused; anything else is a STOP-and-ask. No
pandas on any pipeline path.
...
- `make rebuild` — `ROWS` defaults to `captured`: the anchors seed
  ...
  reviews per month (none — the phase lands no review row, and the line says
  so). Precondition: the developer has run `make scrape CONFIRM=yes` once,
  which fetches the one fetchable source (the Google Play listing) — the
  first time `make scrape` runs green against a live host since the robots
  gate was rebuilt (A1, A6) — and has entered at least the two hand-read rows
  the checks produced (the App Store listing and the Opinion Assurances
  profile, 2026-09-02).
...
1. **`platform_snapshots` lands, seeded from the anchors and marked as such.**
   ...
   exist; the eight anchor rows load from the re-frozen `fixtures/anchors/`
...
3. **Every source declares its parser, cache root, host, profile, segment,
   ...
   Google Play listing (fetchable), the App Store listing and the Opinion
   Assurances profile (declared, not fetchable, each with its terms clause as
   the reason, hand-entered snapshots only); `pipeline/build.py` loads every
...
4. **The first real rows are snapshots — one fetched, the rest read by
   hand.** After the developer's `make scrape CONFIRM=yes` and the entries
   in `data/snapshots/manual_snapshots.csv`, the DONE command prints
   `raw_platform_snapshots` = 8 + the entered rows + the captured listing,
   `raw_reviews` 0 with the one-line reason, the four marts populated, and
   every count unchanged on the second rebuild; a second `make scrape` of the
   unchanged listing adds no raw row. The run is also the first live check of
   the rebuilt robots gate on a host that allows us. *Evidence: row 4.*
5. **The rebuild input is named by what it is, and every frozen sample runs
   ...
   `fixtures/<parser>/` through its real parser, in CI; the new sample
   `fixtures/listings/` is hand-written, fake and nameless with a MANIFEST;
...
6. **The matcher is linear, and no parser or file reads a name.** A robots
   ...
   review item); the manual file carries no address and no name (a source
...
| 6. For all parsers and hand-entered files, no author, pseudonym, name or brand-carrying address is read or stored: the listing parser accepts exactly one `AggregateRating` per page and reads nothing else from the block; the manual file has exactly the eight declared columns, none an address or free text; every frozen sample is fake, nameless and hashes to its MANIFEST. | `tests/test_listing_parser.py::test_only_the_aggregate_rating_is_read`, `::test_two_aggregate_ratings_refuse_the_page`, `::test_no_aggregate_rating_refuses_the_page`, `::test_refusal_names_page_and_field`; `tests/test_snapshots.py::test_manual_file_columns_are_exactly_the_declared_eight`; `tests/test_fixtures_frozen.py::test_manifests_match` |
...
- **A source is a declaration; a parser and a terms position are declared
  ...
  page's `AggregateRating` only; or none, for a source whose snapshots are
  hand-entered only), `page_url(n)`, `pages` (≤ `politeness.MAX_PAGES`, 10),
  ...
  in `politeness.py`, now `("itunes.apple.com", "play.google.com")`; a test
  pins every fetchable source's host is in it). Attribution for reviews:
...
- **The sources, with their positions as checked on 2026-09-02, and the
  ...
  hand-entered.
  (d) the Opinion Assurances profile: `parser=None`, host
  `www.opinion-assurances.fr`, `fetchable=False`, `terms` = conditions
  générales V.3: no automated system or software to extract data from the
  site without prior written authorization; V.1: no reproduction without
  written agreement (read 2026-09-02); robots allows the profile and its
  path-based pages and disallows every address with a query string. The page
  shows the four B1.4 figures. Snapshots hand-entered; **written
  authorization requested by the developer (route 3)** — if granted, the
  letter becomes the terms position and the reviews a later phase's source.
  Peers on that platform may be declared the same way (`parser=None`, a
  traditional or digital-challenger segment) at the developer's choice; each
  is one more hand-read row.
  ...
  guard. The two 2026-09-02 readings (App Store: 4.9, about 13,000 ratings;
  Opinion Assurances: 3.8, 534, 23.1 %, 82 %, 1.5 days) are the first two
  rows — the developer confirms them against the pages before entering.
...
- **`ROWS` names the rebuild input; `samples` runs every frozen sample (D4,
  ...
  exist only in the samples database. Two sample directories: the existing
  ...
  `Robots.parse` and asserts the sample's page address is disallowed), and a
  new hand-written, fake, nameless `fixtures/listings/` (one page in the
  JSON-LD shape with fake values and a placeholder author, with its meta).
  Each has a `MANIFEST.sha256`; `Freeze:` lines below; malformed variants are
...
- `ingest/listing.py` — the JSON-LD `AggregateRating` parser (exactly one
  ...
  read). `ingest/app_store.py` — `parse_page` returns `Parsed`;
...
- `ingest/fetch.py` — `scrape(source, …)` reads host, robots address, page
  ...
  is refused before any request (unchanged). `ingest/robots.py` — `_matches`
  rewritten (pinned decision 6). `ingest/politeness.py` — `ALLOWED_HOSTS =
  ("itunes.apple.com", "play.google.com")`; `MAX_PAGES` stays 10.
- `pipeline/build.py` — `INPUTS = ("captured", "none", "synthetic",
  ...
  hint names the cache root; the no-review line; `scrape` lists the fetchable
...
- `data/snapshots/manual_snapshots.csv` — the header row and the two
  2026-09-02 readings, entered by the developer.
...
- `fixtures/anchors/platform_snapshots_seed.csv` re-frozen with `profile`,
  ...
  re-frozen to the real rule; `fixtures/listings/` new; each with
  `MANIFEST.sha256`.

New and extended tests:
- `tests/test_snapshots.py`, `tests/test_marts.py`,
  `tests/test_listing_parser.py`, `tests/test_fetch_sources.py` new;
...
- [ ] `DECISIONS.md` — Phase 3a entry: the snapshot shape, the anchors
      ...
      listing forbidden by Apple's site terms; Opinion Assurances forbidden
      by its CGU V.3, authorization requested) and the hand-entry path; D1 as
...
- [ ] `BACKLOG.md` — five rows closed (struck + "DONE Phase 3a"): the
      ...
      Opinion Assurances reviews wait on written authorization (trigger: the
      reply); the App Store listing's JSON-LD shape is unverified (trigger:
...
`itunes.apple.com` (declared, refused before any request) and
`play.google.com`; a source on any other host is refused before a request,
and the two non-fetchable sources are refused on their declaration before the
allowlist is consulted. Per run: one `robots.txt` plus one listing page, ≥ 2 s
apart — two requests, about 1.3 MB. Run twice: the same again into a second
capture directory (a public page, no cost, no key); the second rebuild then
adds zero raw rows if the figures are unchanged (invariant 1). No credentials:
none needed, none read (`ingest/` never opens `.env`; the test stands).
Refused (robots disallow, a non-robots body, non-200, timeout): one line
naming host, status and address, no retry, exit 2. Personal data: the listing
page carries a few reviews with display names; it is archived as served under
gitignored `data/`; the parser reads the `AggregateRating` and nothing else.
...
- **Stack risk — the candidate checks, done 2026-09-02 before approval** (from
  ...
  **Disposition, taken:** re-scope to snapshots (this revision). **Still to
  verify in the first hour of the build:** the Play block's field types hold
  on a second day (strings, not numbers — the parser accepts both, strictly);
  a second `make scrape` an hour later adds no raw row when the figures are
  unchanged. Anything else surprising is a STOP; findings go to DECISIONS.md
  → Gotchas.
...
- Trustpilot, its bot handling and any automated path for a refusing host —
  Phase 3b (PLAN §5).
- Real review rows — none of the checked sources allows them. Opinion
  Assurances reviews wait on written authorization (BACKLOG row, trigger: the
  reply); Google Play reviews are behind a disallowed call and are not
  fetched; the App Store feed is disallowed.
