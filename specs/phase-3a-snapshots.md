...
- **Stack risk — the candidate checks, done 2026-09-02 before approval** (from
  ...
  **Disposition, taken:** re-scope to snapshots (this revision), then A1 at
  ...
  session's own analysis of the saved page is blocked): where the review
  date and body sit and in what format the date is written; whether each
  `review` scope carries a stable identifier (an `id`, a `data-*` attribute
  or a permalink) — none is a STOP-and-decide; whether the one-star share,
  response rate and response delay are data (`content`/`itemprop`) or layout
  text; what the page past the last (`-page15.html`) returns. (2) The Play
  block's field types hold on a second day (strings, not numbers — the
  parser accepts both, strictly). (3) A second `make scrape` an hour later
