# Phase 2 — One scraper, end to end (PROPOSED)
...
**Status: PROPOSED — do not start until approved.** One dependency change:
...
| 2. For all feed items, `rating` is the integer 1–5 the feed's digit string denotes and `review_date` is the `YYYY-MM-DD` of the item's own ISO 8601 timestamp; any other value is refused, never defaulted. | `::test_rating_outside_1_to_5_is_refused` (`"0"`, `"6"`, `"4.5"`, `""`, `4`), `::test_malformed_timestamp_is_refused` (`"2025-13-40T00:00:00Z"`, `"yesterday"`, `""`) — a refusal, not a default row |
