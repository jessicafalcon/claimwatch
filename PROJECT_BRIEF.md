...
These are real, sourced public figures gathered during scoping. They seed `platform_snapshots`, validate early pipeline output, and appear in the study as Documented figures marked as seeded (our own later captures are Measured — Phase 3a, decision D3). Each is stored with its source and, where this section gives one, the day it was read; a figure dated only to a month, a season or a span, or not dated, is placed on a day by the rule SPEC.md's Beat 1 states, and that day is a placement, not a reading (`fixtures/anchors/`). A rebuild loads them as stored and does not check them against the live page — our own later captures land beside them as Measured.
