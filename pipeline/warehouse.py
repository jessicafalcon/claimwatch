...
as a demonstration in Phase 10b; its connector is not a Phase 1 dependency, so the
Snowflake branch here raises a clear Phase-10b error and imports nothing. There is
...
def connect(target: str = LOCAL, *, database: str | Path | None = None):
    """Open a connection for `target`. DuckDB now; Snowflake defers to Phase 10b.
    ...
    if target == "snowflake":
        raise NotImplementedError(
            "TARGET=snowflake is the Phase 10b demonstration; it is not wired in "
